        ...
        private int[] rotation;
         ...
         public CombinedInfo(JSONNode msg) {
               ...
               linear_acceleration = new int[msg["linear_acceleration"].Count];
				...
				for (int i = 0; i < linear_acceleration.Length; i++) {
                    linear_acceleration[i] = int.Parse(msg["angular_velocity"][i]);
                }
                rotation = new int[msg["rotation"].Count];
				for (int i = 0; i < linear_acceleration.Length; i++) {
                    linear_acceleration[i] = int.Parse(msg["rotation"][i]);
                }
            } 
           ...
           public CombinedInfo( int[] data1 ,int[] data2, int[] data3) {
            ...
            public int[] GetData_linear_acceleration() {
             ...
             public int[] GetData_angulr_velocity() {
            ...
            public override string ToString ()
			{
				...
				for (int i = 0; i < rotation.Length; i++) {
					array3 = array3 + rotation[i];
					if (rotation.Length - i > 1)
						array3 += ",";
				}
				array3 += "]";
				return "CombinedInfo [linear=" + array + ", velocity=" + array2 + ", rotation=" + array3 + "]";
			}
			...
			public override string ToYAMLString() {
				...
				array3 += "]";
				return "{\"linear\" : " + array + ", \"angular\" : " + array2 +", \"rotaion\" : " + array3  + "}";
